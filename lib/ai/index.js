...
async function persistMessage(threadId, role, text, options = {}) {
  try {
    if (!await getChatById(threadId)) {
      await createChat(options.userId || 'unknown', options.chatTitle || 'New Chat', threadId);
    }
    await saveMessage(threadId, role, text);
  } catch (err) {
...
async function chat(threadId, message, attachments = [], options = {}) {
  ...
  await persistMessage(threadId, 'user', message || '[attachment]', options);
  ...
  await persistMessage(threadId, 'assistant', response, options);
...
async function* chatStream(threadId, message, attachments = [], options = {}) {
  ...
  if (options.repo && options.branch) {
    const existingChat = await getChatById(threadId);
    let workspaceId;

    if (!existingChat) {
      // First message: create workspace row, then chat, then link them
      const { createCodeWorkspace } = await import('../db/code-workspaces.js');
      const workspace = await createCodeWorkspace(options.userId || 'unknown', {
        repo: options.repo,
        branch: options.branch,
      });
      workspaceId = workspace.id;
      await createChat(options.userId || 'unknown', 'New Chat', threadId);
      await linkChatToWorkspace(threadId, workspaceId);
    } else {
  ...
  try {
    ...
    // Save assistant response to DB
    if (fullText) {
      await persistMessage(threadId, 'assistant', fullText, options);
    }
...
async function autoTitle(threadId, firstMessage) {
  try {
    const existingChat = await getChatById(threadId);
    if (!existingChat || existingChat.title !== 'New Chat') return;
    ...
    if (response.title.trim()) {
      await updateChatTitle(threadId, response.title.trim());
    }
  } catch (err) {
