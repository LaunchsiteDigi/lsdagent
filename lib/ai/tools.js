  ...
  return tool(
    async ({ task_description }) => {
      try {
        ...
        const { getCodeWorkspaceById, updateContainerName, updateBranch } = await import('../db/code-workspaces.js');
        const workspace = await getCodeWorkspaceById(workspaceId);
        ...
        const featureBranch = `${slug}-${shortId}`;
        ...
        await updateContainerName(workspaceId, containerName);
        await updateBranch(workspaceId, featureBranch);
