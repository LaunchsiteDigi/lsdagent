...
export async function getCodeWorkspaces() {
  ...
  return await getCodeWorkspacesByUser(user.id);
}
...
export async function createCodeWorkspace(containerName, title = 'Code Workspace') {
  const user = await requireAuth();
  return await dbCreateCodeWorkspace(user.id, { containerName, title });
}
...
export async function renameCodeWorkspace(id, title) {
  const user = await requireAuth();
  const workspace = await getCodeWorkspaceById(id);
  if (!workspace || workspace.userId !== user.id) {
    return { success: false };
  }
  await updateCodeWorkspaceTitle(id, title);
  return { success: true };
}
...
export async function starCodeWorkspace(id) {
  ...
  const starred = await toggleCodeWorkspaceStarred(id);
...
export async function deleteCodeWorkspace(id) {
  ...
  await dbDeleteCodeWorkspace(id);
  return { success: true };
}
