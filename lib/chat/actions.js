...
/**
 * Sync a single chat to the git repository as a markdown file.
 * @param {string} chatId
 * @returns {Promise<{success: boolean, committed?: number}>}
 */
export async function syncConversation(chatId) {
  const user = await requireAuth();
  const chat = getChatById(chatId);
  if (!chat || (chat.userId !== user.id && chat.userId !== 'telegram')) {
    return { success: false };
  ...
  try {
    const { syncChat } = await import('../conversations.js');
...
/**
 * Sync all conversations for the authenticated user to git.
 * @returns {Promise<{success: boolean, committed?: number}>}
 */
export async function syncAllConversations() {
  const user = await requireAuth();
  try {
    const { syncAllChats } = await import('../conversations.js');
    const result = await syncAllChats(user.id);
    // Also sync Telegram chats
    const telegramResult = await syncAllChats('telegram');
    return {
      success: true,
      committed: result.committed + telegramResult.committed,
    };
  } catch (err) {
    console.error('Failed to sync all conversations:', err);
    return { success: false };
  }
}
