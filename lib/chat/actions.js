...
export async function createChat(id, title = 'New Chat') {
  const user = await requireAuth();
  return await dbCreateChat(user.id, title, id);
}
...
export async function getChatTitleByWorkspace(workspaceId) {
  const user = await requireAuth();
  const chat = await getChatByWorkspaceId(workspaceId);
  if (!chat || (chat.userId !== user.id && chat.userId !== 'telegram')) {
    return null;
  }
  return chat.title;
...
export async function starChat(chatId) {
  ...
  const starred = await toggleChatStarred(chatId);
...
export async function deleteAllChats() {
  const user = await requireAuth();
  await deleteAllChatsByUser(user.id);
  return { success: true };
}
...
export async function getAppVersion() {
  ...
  return {
    ...
    changelog: isNewer ? await getReleaseNotes() : null,
  };
}
...
export async function triggerUpgrade() {
  ...
  const targetVersion = await getAvailableVersion();
...
export async function createNewApiKey() {
  ...
  try {
    ...
    return await createApiKeyRecord(user.id);
  } catch (err) {
...
export async function getWorkspace(workspaceId) {
  ...
  try {
    ...
    const ws = await getCodeWorkspaceById(workspaceId);
    if (!ws) return null;
