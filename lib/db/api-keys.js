 ...
 */
export async function createApiKeyRecord(createdBy) {
  const db = await getDb();

  // Delete any existing API key
  await db.delete(settings).where(eq(settings.type, 'api_key')).run();
  ...
  await db.insert(settings).values(record).run();
 ...
 * @returns {Promise<object|null>} Record if valid, null otherwise
 */
export async function verifyApiKey(rawKey) {
  if (!rawKey || !rawKey.startsWith(KEY_PREFIX)) return null;

  const keyHash = hashApiKey(rawKey);
  const cached = await _ensureCache();
  ...
  try {
    const db = await getDb();
    const now = Date.now();
    const row = await db.select().from(settings).where(eq(settings.id, cached.id)).get();
    if (row) {
      const parsed = JSON.parse(row.value);
